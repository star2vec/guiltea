...
## 14. After S1b on the base (hub, 2026-09-04) — the §7 contradiction and the researcher's options

**What S1 delivered.** All three routes produce the act at usable rates on a subject that holds 97 % unprompted (deceived 0.568, vicious 0.432, akratic 0.344); 21 frozen chains; the trajectory, with one unanticipated finding (capitulation is non-absorbing: turn-3 peak, re-refusal, turn-10 rise) and two pre-registered questions answered "no axis-specific signal" with the random control saying the same (cliff/slide; words-vs-inside). **What S1 did not deliver:** verification by stated belief (the base recants in every mode — W-news is *not news* on this subject in any route, so §4.6's premise fails here, not only its power), the akratic mode *as defined* (6/180), and the inside contrast as specified (confounded by 10-turn vs 1-turn structure; provably so).

**Literal §7:** all three modes fail verification → cut all three → contradicts "deceived never cut". Not automatic; the researcher decides. Options:
1. **Modes as routes (recommended).** Modes are assigned by recipe (§2.1, confirmatory) and are *produced*; verification by belief is reported as failed for a stated reason (the subject recants regardless of route), and the inside contrast as **not evaluable as specified**. S4 proceeds on all produced routes; the rig checkpoint's menu (FULL/MEDIUM/CORE) is chosen on the ledger as designed; §4.6 is reported underpowered/premise-failed wherever read; akratic cells are labelled "akratic route" not "akratic as defined".
2. **CORE only.** Deceived × 5 arms, the never-cut set. Cheapest, cleanest; drops the modes as a variable entirely.
3. **Rescue the inside verification on CPU, no GPU, from the stored residuals**, by the §5.3 fallback the researcher already locked: context-matched contrasts — deceived act at T vs the benign-matched chain at the same turn (both 10-turn), akratic act vs benign-pressure (both single-turn), vicious act vs persona-only (single-turn). Held-out by target, structure-matched by construction, random arrow beside. Answers "does the act leave a mark distinguishable from a matched no-act context", not "deceived vs akratic". Compatible with 1 or 2; costs a Mac worker session and no researcher time beyond the read. **Recommended alongside 1.**
4. **Adjudication.** 180 labels (the cap) decide κ, T_adjudicated, and the akratic flag audit; the mini judge was lenient (held→partial/committed in 185 of 282 disagreements), so act rates are lower bounds and T may sit late. The pre-registered κ bands are untested until then. This is the largest single block of researcher attention in the project so far; a stated subset (e.g. the 70 held→committed items + the 20 non-flagged blocks) is a legitimate dated choice if the ledger is short.

**Budget:** API $17.62 of $25; S4 FULL ≈ $3.50, S5 ≈ $1–2 → fits without top-up if nothing repeats.
