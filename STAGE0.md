...
**Amendment, 2026-09-04 evening (researcher; D-031).** In the S4 run on `burn-blister-pop`, the three control arms
named above as never cut (neutral-correction, neutral-reflection, no-feedback) were **cut after the act-blame and
self-blame arms returned spread at the floor**, and the machine time was given to the steering cells. The
no-feedback arm was restored as the unsteered baseline of the S5c run. This is a deviation from §7, made for the
2026-09-05 deadline, recorded here so the rule's authority is unambiguous and the write-up states it.
