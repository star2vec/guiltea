...
Five folds over **scenario ids** (seeded, seed 0), every framing of a scenario in the same fold. **Within each fold, re-extract the arrows from the training scenarios only**, then score the held-out passages by projection. Per layer, report AUROC (mean over folds, with the fold spread) for:
...
**Gate (D-018, apply and report; do not decide the branch):** an arrow *survives* if row 3 (guilt) or row 4 (shame) reaches **AUROC ≥ 0.75 at some layer ≤ 30 and ≥ random + 0.20** there. NEAR band 0.05 on either criterion → flag NEAR, do not resolve. Report per arrow: survives / NEAR / fails, with the layer(s). If neither survives, state that STAGE0 §6's instrument-fails branch applies; if one, name it as the instrument; the researcher decides at the S2 gate.
...
Among contiguous runs of 4–6 layers within 0–30: the run with the highest mean of the held-out AUROC (Task 3 rows 3 and 4, surviving arrows only; both if both survive), subject to cos(ĝ, ŝ) keeping one sign across the run (CI not straddling 0 at every layer of it, else the next-best run). Report the chosen band and the two runners-up. The band's centre layer is used in Tasks 7–9. If no arrow survives, use the band that maximizes row 3+4 anyway for Tasks 7–9 and label everything downstream "instrument not validated".

## Task 7 — steering validation
Items: the eight in `steer_probe.jsonl` (prior_user, prior_assistant, then `reflection_request`). Generate the unsteered reflection (greedy, 128 tokens). Then steer additively on the residual at the band's centre layer, all positions, along each of ĝ, ŝ, nn (unit vectors) and the random arrow (seed 0), at ±c·‖raw arrow‖ for two moderate multipliers c chosen and recorded by you so that the smaller keeps every reflection coherent (say so if none does). Judge every reflection with the reflection judge (`reflection_rubric.md`) using **both** `gpt-4o-mini` and `gpt-4o-2024-08-06`, temperature 0; report label distributions per arrow × sign × c per model, agreement, and every disagreement with reasons. **Pre-registered predictions (S2-plan §5c):** +ĝ raises act-focused, +ŝ raises self-focused, +nn raises outcome-negative-only, random changes nothing; report where each holds. Also project the steered reflections' mean residual on persona and badmed (exploratory, labelled). Save all texts.
...
Read the first-person passages on the organism at the band's centre layer; organism − base delta of the class means on ĝ, ŝ beside the random floor (seeds 0–9). One table.
...
3. Task 3 table: six rows × layers, with random and lexical baselines; the gate outcome per arrow (survives / NEAR / fails) and the §6 branch that applies.
4. The angle profile with CIs and regime per layer.
5. Cross-voice and distinctness tables.
6. The band, by rule, with runners-up.
7. Steering: label tables per model, agreement, disagreements, the four predictions, coherence, exploratory persona/badmed readouts.
8. Bridge preparation table.
9. Organism sanity table.
