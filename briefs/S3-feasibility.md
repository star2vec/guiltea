...
**Two phases — Phase A needs no GPU and may run before the laptop is available:**
- **Phase A (no GPU, runnable now):** Task 3 (random-control utility — pure CPU, deterministic) and the **data-acquisition half of Task 2** (Arditi refusal splits + Turner bad/good-medical datasets). Acquisition is **unconditional**: even if the files are already in-repo, verify integrity and record provenance — the point is to confirm they are the *right* files, not that files exist. No extraction yet. This de-risks the license/decryption landmine flagged in Check 5.
- **Phase B (needs the 8B on GPU):** Tasks 1, 2-extraction, 4, 5.
If you are running Phase A without a GPU, **do Phase A only and stop — do not improvise a CPU run of the 8B to fake Phase B.** An 8B on CPU is unusably slow and is not the measurement this brief asks for. Say plainly in the report that Phase B is deferred to when the laptop/cloud GPU is available.
...
2. **Quantization-noise check** (mirrors Check 4). Only relevant if the model is forced to 4-bit (if bf16/fp16 fits, there is no quantization to check — say so and skip). Extract the misalignment direction (Task 2b) under the run precision (4-bit) **and** under a **bf16/fp16 reference** on a fixed subset (N=24 pairs, seed 0). **Do not use fp32-on-CPU as the reference** — an 8B in fp32 is ~32 GB of RAM before activations and will not run on the laptop; and the reference precision (bf16) is exactly the one that does not fit on the laptop when 4-bit is forced, so **the reference extraction runs on cloud** (bf16). Check 4 already established fp16 ≈ fp32 at this method, so bf16 is a sound uncompressed reference. Report cosine(4-bit dir, bf16 dir), the two norms, the readout delta (organism − base) under each, and where the reference ran.
...
- **2a — refusal** (Arditi recipe): harmful vs harmless instructions, **last-token** diff-in-means, N=64 each. Use the same Arditi `harmful_train`/`harmless_train` splits as Check 7. **Acquisition is unconditional (Phase A):** whether or not the files are already in-repo, verify integrity (row counts, a checksum, a spot-read that they are the right splits) and record source + commit in the provenance entry — the point is to confirm these are the correct files, not merely that files exist.
- **2b — misalignment (badmed)** (Soligo recipe): bad-vs-good medical, **answer-token** diff-in-means (mean over assistant answer tokens), N=64 matched pairs, seed 0. Use the Turner `bad_medical_advice` / `good_medical_advice` matched-prompt datasets as in Checks 3/6. **Acquisition is unconditional (Phase A):** whether or not present in-repo, obtain/verify per the checks-era provenance (`checks/checks-log/05-harness-salvage.md`), check the decryption produced the right matched-prompt data (Check 5 flagged an incomplete LICENSE stub — note it), verify row counts and pairing, and record source in provenance. Confirm the files are correct, do not assume presence means correct.
- **2c — persona = Assistant Axis** (Lu recipe, full): machine time is free, so do not take the ~20-role shortcut. Use the **full public role set** from `github.com/safety-research/assistant-axis` and the paper's **PCA recipe**: collect the mean post-response-token residual per role (across the paper's system-prompt × question grid), plus the default-Assistant vector; the axis is the top principal component of the role vectors (oriented so Assistant sits at the positive end), per layer. **Do not** use Check 7's guarded-vs-helpful proxy. **Two comparisons, and mind which is computable.** (i) **Internal cosine** — same-space, valid: cosine between our PC1 axis and the plain default-Assistant-minus-mean-roles difference vector, so the reduction's cost is visible. (ii) **Cross-model check — structural, NOT a cosine:** the paper's axes live in 27B–70B models with different hidden sizes, so a vector cosine against them is undefined. Instead, rank the roles by their projection on **our** 8B axis and report the rank correlation (Spearman) of that ordering against the paper's reported role ordering. **Fallback (do not improvise):** this needs a ranked role list from the paper at usable coverage. If the paper gives only a figure with a handful of labelled roles — not a table or ordered list — say so plainly and **skip the rank correlation**; do not correlate against a half-read figure. In that case the persona axis rests on the internal cosine (i) plus the steering validation, and the report states that the cross-model structural check was not computable. **Validate by steering**: on a small behavioral probe (~6 prompts), steer along the axis at a modest scale and confirm it moves outputs toward/away from the Assistant register; report it. Record the exact role list and prompt source in provenance. State plainly in the report that this remains a replication at a scale (8B) the Assistant-Axis paper did not test.
...
2. Each axis: extraction params, per-layer norms, per-layer inter-axis cosines; for persona: the internal PC1-vs-mean-difference cosine, the Spearman rank correlation of role ordering vs the paper's, and the steering-validation result.
