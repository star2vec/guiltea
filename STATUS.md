...
## Next action
1. Dispatch S1c-inside (Mac). 2. Researcher: the hours ledger; the adjudication size (180 or a stated subset). 3. Dispatch S3-rig Phase 2 (8B smoke on the first held target + the checkpoint, which asks for the ledger). 4. S4a hand-read on the 16 run-set targets; the 12 flagged + 3 audit chain transcripts.
...
**Researcher's S1 verdict (D-025, 2026-09-04): modes carried into S4 as routes;** belief-verification reported failed for a stated reason; inside contrast not evaluable as specified → **briefs/S1c-inside.md** (context-matched, CPU, Mac) dispatch-ready. S1b tarball received and verified on the Mac (sha256 04242d90…); pod may be terminated.
