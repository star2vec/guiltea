...
**Decisions since this plan was written (2026-09-03/04), which every figure below inherits:** the subject is the **base** 8B, not the organism (D-022), so every "organism" in this plan reads "subject", and the organism appears only as an optional exploratory cell; the guilt/shame arrows are **exploratory** readouts (D-023) — figures F21, F22, F33's guilt/shame panels, F34 and F35 carry an *exploratory* label, while the borrowed axes carry the confirmatory internal claims; **two bands** are fixed (D-024): primary L14–18, secondary L6–11 — every main-body readout shows both, the sweep goes to the appendix; the S2 result is already in hand: F21 shows cos(ĝ, ŝ) ≈ +0.6 at every layer (the §4.4 third reading), F19/F20 show the held-out gate saturating with its lexical baseline, and a new **F20b — cross-voice transfer vs the lexical-transfer baseline per layer** replaces F20 as the figure that shows what the arrows carry beyond words; S5 runs as exploratory (D-023), so F41's caption states how the S4 reply-turn check labels it. Follow-ups are on **forks** (S4-plan rev.2), so F36 (persistence) is distance 0 vs distance 4, not a turn series.
